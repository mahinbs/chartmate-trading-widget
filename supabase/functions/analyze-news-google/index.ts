...
function stripExchangeSuffix(sym) {
  return sym.replace(/\.(NS|BO|NSE|BSE)$/i, "").replace(/^NSE:/i, "").replace(/^BSE:/i, "").trim();
}
function searchQueryForNews(symbol) {
...
async function fetchFinnhubNews(symbol) {
  ...
  try {
    ...
    return arr.slice(0, 15).map((item)=>({
        time: item.datetime ? new Date(item.datetime * 1000).toISOString() : new Date().toISOString(),
        source: item.source || "Finnhub",
        headline: item.headline || item.summary || "",
        sentiment_score: typeof item.sentiment === "number" ? item.sentiment : 0,
...
async function fetchNewsDataSymbol(symbol) {
  const key = Deno.env.get("NEWSDATA_API_KEY");
  if (!key) return [];
  const q = stripExchangeSuffix(symbol);
  try {
    const url = `https://newsdata.io/api/1/news?apikey=${key}&q=${encodeURIComponent(q)}&language=en&size=10`;
    const res = await fetch(url, {
      signal: AbortSignal.timeout(12000)
    });
    if (!res.ok) return [];
    ...
    return data.results.map((article)=>({
        time: article.pubDate ? new Date(article.pubDate).toISOString() : new Date().toISOString(),
        source: article.source_id || "NewsData.io",
        headline: article.title || "",
        sentiment_score: 0,
        novelty: Array.isArray(article.category) ? String(article.category[0]) : "News",
        relevance: "0.9",
        url: typeof article.link === "string" && article.link.startsWith("http") ? article.link : undefined
      }));
  } catch (e) {
...
async function fetchMarketAuxSymbol(symbol) {
  const key = Deno.env.get("MARKETAUX_API_KEY") || Deno.env.get("MARKET_AUX_API");
  if (!key) return [];
  const clean = stripExchangeSuffix(symbol);
  try {
    const url = `https://api.marketaux.com/v1/news/all?symbols=${encodeURIComponent(clean)}&api_token=${key}&limit=12&language=en`;
    const res = await fetch(url, {
      signal: AbortSignal.timeout(12000)
    });
    if (!res.ok) return [];
    ...
    return rows.map((article)=>({
        time: article.published_at || new Date().toISOString(),
        source: article.source || "MarketAux",
        headline: article.title || "",
        sentiment_score: typeof article.sentiment === "number" && article.sentiment !== 0 ? article.sentiment : typeof article.sentiment_score === "number" ? article.sentiment_score : 0,
        novelty: "Markets",
        relevance: "0.95",
        url: typeof article.url === "string" && article.url.startsWith("http") ? article.url : undefined
      }));
  } catch (e) {
...
async function fetchEnhancedNewsData(symbol) {
  const newsItems = [];
  try {
  ...
  try {
    ...
    const mapFeed = (data)=>{
      ...
      return data.feed.slice(0, 10).map((item)=>({
          ...
          relevance: String(item.ticker_sentiment?.[0]?.relevance_score ?? '0.0'),
          url: typeof item.url === 'string' && item.url.startsWith('http') ? item.url : undefined
        }));
    };
    const out = [];
    for (const t of [
      ...
      const response = await fetch(`https://www.alphavantage.co/query?function=NEWS_SENTIMENT&tickers=${encodeURIComponent(t)}&apikey=${alphaVantageKey}&limit=20`, {
...
serve(async (req)=>{
  ...
  try {
    ...
    const overallSentiment = newsData.length > 0 ? newsData.reduce((sum, item)=>sum + item.sentiment_score, 0) / newsData.length : 0;
    ...
    const summaryLines = newsData.length > 0 ? [
      ...
      geminiRows > 0 ? `${geminiRows} row(s) used Gemini with Google Search grounding (real URLs when the API returns citation chunks).` : "",
      withUrl > 0 ? `${withUrl} item(s) open to a publisher page in a new tab.` : "Few or no direct URLs in this batch. Set GEMINI_API_KEY for search-grounded links, or add NewsData, MarketAux, or Finnhub.",
      "News shapes sentiment in the model; it does not replace chart or liquidity risk."
    ].filter(Boolean).join(" ") : "No recent news found";
    const result = {
      symbol,
      ...
      newsItems: newsData,
      sources: [
        ...new Set(newsData.map((item)=>item.source))
      ],
