...
serve(async (req)=>{
  ...
  try {
    ...
    if (articleId) {
      const { data: cached } = await supabaseClient.from('news').select('title, full_content, author').eq('id', articleId).single();
      if (cached) {
    ...
    async function getFinalUrlAndHtml(initialUrl) {
    ...
    if (!articleTitle) {
      articleTitle = $('meta[property="og:title"]').attr("content") || $('meta[name="twitter:title"]').attr("content") || $('title').text().trim() || "";
    }
    let finalImageUrl = $('meta[property="og:image"]').attr("content") || $('meta[name="twitter:image"]').attr("content") || null;
            ...
            contents: [
              {
                parts: [
                  {
                    text: `Generate a photorealistic professional header image for a news article titled: "${articleTitle}".`
                  }
                ]
              }
            ]
