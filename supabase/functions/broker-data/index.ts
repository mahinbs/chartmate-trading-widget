...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: integration } = await supabase.from("user_trading_integration").select("openalgo_api_key, strategy_name").eq("user_id", user.id).eq("is_active", true).maybeSingle();
    const apiKey = integration?.openalgo_api_key ?? "";
    const strategyName = integration?.strategy_name ?? "ChartMate AI";
    if (!apiKey.trim()) {
      return new Response(JSON.stringify({
        error: "No active broker integration.",
    ...
    if (action === "orderstatus") {
      ...
      return new Response(JSON.stringify({
        error: "action must be: orderstatus | quotes | multiquotes | margin"
      }), {
        status: 400,
        headers
      });
    ...
    if (!oaRes.ok) {
      const detail = oaData?.message ?? oaData?.error ?? `OpenAlgo ${oaRes.status}`;
      return new Response(JSON.stringify({
        error: detail,
        raw: oaData
      }), {
