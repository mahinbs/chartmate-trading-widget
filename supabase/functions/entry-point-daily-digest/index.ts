...
import { getDigestClockContext, isInDigestFireWindow, normalizeTimezoneForDigest, parseNotifyTimeForDigest } from "../_shared/digestClock.ts";
const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Methods": "POST, OPTIONS",
  "Access-Control-Allow-Headers": "authorization, content-type"
};
function scheduleAllows(row, weekday, dateKey) {
...
async function processOneTracker(supabase, row, secret, fnUrl, now, timeMode) {
  ...
  const timeOk = timeMode === "exact_minute" ? nowMin === tgtMin : isInDigestFireWindow(nowMin, tgtMin);
  if (!timeOk) return "skipped";
  const prevLast = row.last_digest_on;
  const { data: claimedRows, error: claimErr } = await supabase.from("live_entry_trackers").update({
    last_digest_on: dateKey
  }).eq("id", row.id).or(`last_digest_on.is.null,last_digest_on.neq.${dateKey}`).select("id");
  if (claimErr || !claimedRows?.length) return "skipped";
  const revertLast = async ()=>{
    await supabase.from("live_entry_trackers").update({
      last_digest_on: prevLast ?? null
    }).eq("id", row.id);
  };
  try {
    const selectedStrategies = Array.isArray(row.selected_strategies) && row.selected_strategies.length > 0 ? row.selected_strategies : [
      "trend_following",
      "mean_reversion",
      "momentum"
    ];
    let customStrategiesPayload = [];
    const customIds = Array.isArray(row.selected_custom_strategy_ids) ? row.selected_custom_strategy_ids : [];
    if (customIds.length > 0) {
      const { data: customRows } = await supabase.from("user_strategies").select("id,name,description,trading_mode,stop_loss_pct,take_profit_pct,is_intraday,paper_strategy_type,entry_conditions,exit_conditions,position_config,risk_config,chart_config,execution_days,market_type,start_time,end_time,squareoff_time,risk_per_trade_pct").eq("user_id", row.user_id).in("id", customIds);
      customStrategiesPayload = (customRows ?? []).map((cs)=>({
    ...
    const historyId = typeof payload.historyId === "string" ? String(payload.historyId) : null;
    ...
    const message = signals.length === 0 ? `Entry digest — no scored signals right now for ${row.symbol}. Open Live Trading → Scanner to rescan.` : `Entry digest — ${live} live · ${past} past · ${buy} BUY / ${sell} SELL (${signals.length} total). Tap Scanner for full breakdown.`;
...
Deno.serve(async (req)=>{
  ...
  if (!isCron) {
    if (!trackerId) {
      return new Response(JSON.stringify({
        error: "trackerId required when using session auth"
      }), {
        status: 400,
        headers
      });
    ...
    const { data: row, error: rowErr } = await supabase.from("live_entry_trackers").select("id,user_id,symbol,display_name,notify_time,timezone,enabled,last_digest_on,schedule_mode,days_of_week,one_off_local_date,selected_strategies,selected_custom_strategy_ids").eq("id", trackerId).eq("user_id", uid).maybeSingle();
    if (rowErr || !row) {
      return new Response(JSON.stringify({
    ...
    if (!secret) {
      return new Response(JSON.stringify({
        error: "ENTRY_DIGEST_SECRET not configured"
      }), {
        status: 500,
        headers
      });
    ...
    return new Response(JSON.stringify({
      ok: true,
      outcome: outcome === "sent" ? "sent" : "skipped"
    }), {
      status: 200,
      headers
    });
  ...
  const { data: rows, error: qErr } = await supabase.from("live_entry_trackers").select("id,user_id,symbol,display_name,notify_time,timezone,enabled,last_digest_on,schedule_mode,days_of_week,one_off_local_date,selected_strategies,selected_custom_strategy_ids").eq("enabled", true);
  if (qErr) {
