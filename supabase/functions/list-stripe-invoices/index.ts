...
type InvoiceOut = {
  ...
  number: string | null;
  status: string | null;
  /** Total in smallest currency unit (invoice total). */
  total: number;
  amount_paid: number;
  currency: string;
  created: number;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const customerId = String((row as { stripe_customer_id?: string } | null)?.stripe_customer_id ?? "").trim();
    if (!customerId || customerId.startsWith("cus_manual_exc_")) {
      return new Response(JSON.stringify({ invoices: [] as InvoiceOut[] }), { status: 200, headers });
    }

    const params = new URLSearchParams({ customer: customerId, limit: "24" });
    const stripeRes = await fetch(`https://api.stripe.com/v1/invoices?${params}`, {
      headers: { Authorization: `Bearer ${STRIPE_SECRET}` },
    });
    const stripeData = await stripeRes.json().catch(() => ({}));
    if (!stripeRes.ok) {
      const msg = (stripeData as { error?: { message?: string } }).error?.message ?? "Stripe invoice error";
      console.error("list-stripe-invoices:", msg);
      return new Response(JSON.stringify({ error: msg }), { status: 400, headers });
    }

    const rawList = (stripeData as { data?: unknown[] }).data ?? [];
    const invoices: InvoiceOut[] = rawList.map((inv: unknown) => {
