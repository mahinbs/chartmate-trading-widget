...
function getIncrementForPrice(price, asset) {
...
function buildLevelsFromQuote(price, change, volume, asset) {
  ...
  for(let i = -12; i <= 12; i++){
    ...
    const bid = i <= 0 ? base * weight * (directionalBias > 0 ? 1.2 : 0.95) : base * weight * 0.08;
    const ask = i >= 0 ? base * weight * (directionalBias > 0 ? 0.95 : 1.2) : base * weight * 0.08;
    out.push({
      price: p,
      bid,
      ask
...
function toTiingoFxTicker(symbol) {
  const s = symbol.toUpperCase().trim();
...
function parseTiingoFxTick(raw) {
  try {
...
async function fetchTiingoFxSnapshot(symbol) {
...
function buildSyntheticHistoryFromSnapshot(base, timeframe, count) {
...
function defaultForexMid(symbol) {
...
function buildFallbackForexSnapshot(symbol) {
...
function defaultStockMid(symbol) {
  const s = symbol.toUpperCase();
...
function buildFallbackStockSnapshot(symbol) {
...
function mapTimeframeToFinnhub(tf) {
  const x = tf.toLowerCase();
  if (x === "1m") return "1";
  if (x === "5m") return "5";
  if (x === "30m") return "30";
...
async function fetchYahooIndian(symbol) {
  ...
  for(let i = Math.max(0, rows.length - 30); i < rows.length; i++){
    ...
    const old = levelsMap.get(bucket) ?? {
      price: bucket,
      bid: 0,
      ask: 0
    };
...
async function fetchYahooIndianHistory(symbol, timeframe, count) {
...
async function fetchYahooMarketHistory(symbol, asset, timeframe, count) {
  const interval = mapTimeframeToYahoo(timeframe);
  const sec = timeframeToSeconds(timeframe);
  const period2 = Math.floor(Date.now() / 1000);
  const period1 = period2 - sec * Math.max(40, count + 10);
...
async function fetchYahooMarketSnapshot(symbol, asset) {
  const period2 = Math.floor(Date.now() / 1000);
...
async function fetchFinnhub(symbol, asset) {
...
async function fetchFinnhubHistory(symbol, asset, timeframe, count) {
  if (!FINNHUB_API_KEY) throw new Error("FINNHUB_API_KEY missing");
  const fs = toFinnhubSymbol(symbol);
...
async function fetchBinanceHistory(symbol, timeframe, count) {
  const pair = symbol.endsWith("-USD") ? `${symbol.replace("-USD", "")}USDT` : symbol.endsWith("-USDT") ? symbol.replace("-", "") : "BTCUSDT";
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!symbol) return new Response(JSON.stringify({
    ...
    const tRes = await fetch(`https://api.binance.com/api/v3/ticker/bookTicker?symbol=${pair}`, {
