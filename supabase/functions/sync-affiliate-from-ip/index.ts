...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if ((prof as { affiliate_id?: string } | null)?.affiliate_id) {
      return new Response(JSON.stringify({ ok: true, skipped: "already_attributed" }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (typeof metaAff === "string" && metaAff.trim()) {
      const { data: affOk } = await supabase
        ...
        .maybeSingle();
      if (affOk?.id) {
        await applyAffiliateToUserProfileIfEmpty(supabase, user.id, user.email, affOk.id as string);
        return new Response(JSON.stringify({ ok: true, source: "metadata" }), {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    if (!fromIp) {
      return new Response(JSON.stringify({ ok: true, skipped: "no_visitor_match" }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    return new Response(JSON.stringify({ ok: true, source: "visitor_ip" }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
