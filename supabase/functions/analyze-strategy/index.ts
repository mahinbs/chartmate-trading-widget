...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // Fetch strategy
    const { data: strategy, error: fetchErr } = await supabase.from("user_strategies").select("*").eq("id", strategyId).eq("user_id", user.id).maybeSingle();
  ...
  "notes": "<key insight for the trader in 1-2 sentences>"
    ...
    if (!aiAnalysis) {
      ...
      aiAnalysis = {
        ...
        optimal_market_conditions: s.trading_mode === "LONG" ? "Trending bull market, sector rotation into your symbols" : s.trading_mode === "SHORT" ? "Bearish trend, weak sector performance, high volatility" : "Any trending market with clear direction",
    ...
    const winRateEst = (aiAnalysis.win_rate_estimate_pct ?? 55) / 100;
