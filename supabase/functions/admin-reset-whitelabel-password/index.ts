...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: tenant, error: tenantErr } = await supabaseClient.from("white_label_tenants").select("owner_user_id, owner_email").eq("id", tenantId).maybeSingle();
    if (tenantErr) throw new Error(tenantErr.message);
    if (!tenant?.owner_user_id) {
      return new Response(JSON.stringify({
        error: "Tenant owner user not found"
      }), {
        status: 404,
        headers: {
          ...corsHeaders,
