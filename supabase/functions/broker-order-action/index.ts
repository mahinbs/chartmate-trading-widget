...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: integration } = await supabase.from("user_trading_integration").select("openalgo_api_key, strategy_name, is_active").eq("user_id", user.id).eq("is_active", true).maybeSingle();
    const apiKey = integration?.openalgo_api_key ?? "";
    const strategyName = integration?.strategy_name ?? "ChartMate AI";
    if (!apiKey.trim()) {
      return new Response(JSON.stringify({
        error: "No active broker integration. Please reconnect your broker.",
    ...
    if (!action) {
      return new Response(JSON.stringify({
        error: "action is required: cancel | modify | close_all_pos | cancel_all"
      }), {
        status: 400,
        headers
      });
    ...
    if (action === "cancel") {
      ...
      if (!orderid) {
        return new Response(JSON.stringify({
          error: "orderid is required for cancel"
        }), {
          status: 400,
          headers
        });
      ...
      const { orderid, symbol, exchange, order_action, product, pricetype, price, quantity } = body;
      if (!orderid || !symbol || !price || !quantity || !order_action) {
        return new Response(JSON.stringify({
          error: "orderid, symbol, order_action, price, and quantity are required for modify"
        }), {
          status: 400,
          headers
        });
    ...
    await supabase.from("order_audit_logs").insert({
      ...
      error_message: oaRes.ok ? null : oaData?.message ?? `${oaRes.status}`
    }).catch((e)=>console.warn("audit log error:", e));
    if (!oaRes.ok) {
      const detail = oaData?.message ?? oaData?.error ?? `OpenAlgo returned ${oaRes.status}`;
      return new Response(JSON.stringify({
        error: detail,
        raw: oaData
      }), {
