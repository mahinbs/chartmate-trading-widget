...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const onboardingId = (body.onboarding_id ?? "").trim();
    if (!onboardingId) {
      return new Response(JSON.stringify({
    ...
    if (onboarding.status === "provisioned" || onboarding.status === "active") {
      return new Response(JSON.stringify({
        error: "This user has already been provisioned"
      }), {
        status: 409,
        headers
      });
    }
    const targetUserId = onboarding.user_id;
    // ── Get user's email from Supabase auth ───────────────────────────────
    const { data: { user: targetUser }, error: userErr } = await supabase.auth.admin.getUserById(targetUserId);
    if (userErr || !targetUser) {
      return new Response(JSON.stringify({
        error: "Could not fetch user email from auth"
      }), {
        status: 500,
        headers
      });
    ...
    if (!OPENALGO_URL) {
      return new Response(JSON.stringify({
        error: "OPENALGO_URL not configured in Supabase secrets"
      }), {
        status: 503,
        headers
      });
    ...
    if (!APP_KEY) {
      return new Response(JSON.stringify({
        error: "OPENALGO_APP_KEY not configured in Supabase secrets"
      }), {
        status: 503,
        headers
      });
    ...
    const { data: integrationRow, error: upsertErr } = await supabase.from("user_trading_integration").upsert({
    ...
    if (upsertErr) {
      ...
      return new Response(JSON.stringify({
        error: "Failed to save integration: " + upsertErr.message
      }), {
        status: 500,
        headers
      });
    ...
    const { error: assignmentErr } = await supabase.from("algo_user_assignments").upsert({
      user_id: targetUserId,
    ...
    if (assignmentErr) {
      ...
      return new Response(JSON.stringify({
        error: "Failed to save user assignment: " + assignmentErr.message
      }), {
        status: 500,
        headers
      });
    ...
    await supabase.from("algo_onboarding").update({
      status: "provisioned",
    ...
    return new Response(JSON.stringify({
      ...
      openalgo_username: openalgoUsername,
      openalgo_created: oaData?.created ?? true
    }), {
      status: 200,
      headers
    });
