...
async function assertAdmin(userId, supabaseClient) {
...
serve(async (req)=>{
  ...
  try {
    ...
    if (req.method === "GET") {
      const { data, error } = await supabaseClient.from("admin_symbol_watchlist").select("*").order("sort_order", {
        ascending: true
      });
    ...
    if (req.method === "PUT") {
      ...
      const { error: deleteError } = await supabaseClient.from("admin_symbol_watchlist").delete().not("id", "is", null);
