  ...
  try {
    if (!OPENALGO_URL) {
      return new Response(JSON.stringify({
        error: "OPENALGO_URL not configured. Contact support."
      }), {
        status: 503,
        headers
      });
    ...
    const { symbol, action, quantity, exchange, product, pricetype, price, strategy } = body;
    const strategyCode = (body.strategy_code ?? "").trim().toLowerCase();
    const orderIntent = body.intent === "exit" ? "exit" : "entry";
    const tradeId = (body.trade_id ?? "").trim() || null;
    ...
    const { data: assignment } = await supabase.from("algo_user_assignments").select("allowed_strategy, status, integration_id").eq("user_id", user.id).eq("status", "active").maybeSingle();
    ...
    const INDIAN_EQUITY_EXCHANGES = new Set([
      "NSE",
      "BSE"
    ]);
    const INDIAN_DERIV_EXCHANGES = new Set([
      "NFO",
      "BFO",
      "CDS",
      "BCD",
    ...
    const INDIAN_COMMODITY_EXCHANGES = new Set([
      "MCX",
      "NCDEX",
      "NCE"
    ]);
    const isIndianExchange = INDIAN_EQUITY_EXCHANGES.has(resolvedExchange) || INDIAN_DERIV_EXCHANGES.has(resolvedExchange) || INDIAN_COMMODITY_EXCHANGES.has(resolvedExchange);
    ...
    const resolvedStrategyName = strategy ?? access.integration.strategy_name ?? (strategyCode || "ChartMate AI");
    const orderPayload = {
    ...
    if (!openalgoRes.ok) {
      const detail = openalgoData?.message ?? openalgoData?.error ?? "Order failed";
    ...
    return new Response(JSON.stringify(openalgoData), {
      status: 200,
      headers
    });
  } catch (err) {
    ...
    return new Response(JSON.stringify({
      error: "Internal error placing order"
    }), {
      status: 500,
      headers
    });
