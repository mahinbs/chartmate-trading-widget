...
function checkStrategyAchieved(strategy, action, latestClose, latestSMA20, latestRSI, high20d, low20d) {
  ...
  switch(strategy){
    case "trend_following":
      if (action === "BUY") return aboveSMA && rsi > 45 ? {
        achieved: true,
        reason: `Price (${latestClose.toFixed(2)}) is above SMA20 (${latestSMA20?.toFixed(2)}) and RSI ${rsi.toFixed(1)} > 45 — uptrend conditions met.`
      } : {
        achieved: false,
        reason: `Trend-following BUY requires price above SMA20 and RSI > 45. Current: price ${aboveSMA ? "above" : "below"} SMA20, RSI ${rsi.toFixed(1)}.`
      };
      return belowSMA && rsi < 55 ? {
        achieved: true,
        reason: `Price below SMA20 and RSI ${rsi.toFixed(1)} < 55 — downtrend conditions met for SELL.`
      } : {
        achieved: false,
        reason: `Trend-following SELL requires price below SMA20. Current: price ${belowSMA ? "below" : "above"} SMA20.`
      };
    case "breakout_breakdown":
      if (action === "BUY") return pctFromHigh > -2 ? {
        achieved: true,
        reason: `Price is within 2% of the 20-day high (${high20d.toFixed(2)}) — breakout setup is active.`
      } : {
        achieved: false,
        reason: `Breakout BUY requires price within 2% of the 20-day high (${high20d.toFixed(2)}). Current price is ${Math.abs(pctFromHigh).toFixed(1)}% away.`
      };
      return pctFromLow < 2 ? {
        achieved: true,
        reason: `Price is within 2% of the 20-day low (${low20d.toFixed(2)}) — breakdown setup is active.`
      } : {
        achieved: false,
        reason: `Breakdown SELL requires price within 2% of the 20-day low (${low20d.toFixed(2)}).`
      };
    case "mean_reversion":
      if (action === "BUY") return rsi < 35 ? {
        achieved: true,
        reason: `RSI ${rsi.toFixed(1)} < 35 — asset is oversold, mean-reversion BUY conditions met.`
      } : {
        achieved: false,
        reason: `Mean-reversion BUY requires RSI < 35 (oversold). Current RSI: ${rsi.toFixed(1)}. Wait for a deeper pullback.`
      };
      return rsi > 65 ? {
        achieved: true,
        reason: `RSI ${rsi.toFixed(1)} > 65 — asset is overbought, mean-reversion SELL conditions met.`
      } : {
        achieved: false,
        reason: `Mean-reversion SELL requires RSI > 65 (overbought). Current RSI: ${rsi.toFixed(1)}.`
      };
    case "momentum":
      if (action === "BUY") return rsi > 55 && aboveSMA ? {
        achieved: true,
        reason: `RSI ${rsi.toFixed(1)} > 55 and price above SMA20 — momentum is bullish, conditions met.`
      } : {
        achieved: false,
        reason: `Momentum BUY requires RSI > 55 and price above SMA20. RSI: ${rsi.toFixed(1)}, price ${aboveSMA ? "above" : "below"} SMA20.`
      };
      return rsi < 45 && belowSMA ? {
        achieved: true,
        reason: `RSI ${rsi.toFixed(1)} < 45 and price below SMA20 — bearish momentum conditions met.`
      } : {
        achieved: false,
        reason: `Momentum SELL requires RSI < 45 and price below SMA20.`
      };
    case "scalping":
      {
        ...
        return intraVolatility > 0.3 ? {
          achieved: true,
          reason: `Intraday price movement of ~${intraVolatility.toFixed(1)}% from SMA — sufficient volatility for scalping.`
        } : {
          achieved: false,
          reason: `Scalping needs higher intraday volatility. Price is too close to average — spreads may eliminate profits.`
        };
      }
    case "swing_trading":
      if (action === "BUY") return aboveSMA && rsi > 40 && rsi < 65 ? {
        achieved: true,
        reason: `RSI ${rsi.toFixed(1)} in neutral zone (40–65) with price above SMA20 — healthy pullback for swing entry.`
      } : {
        achieved: false,
        reason: `Swing-trading BUY needs price above SMA20 and RSI between 40–65. RSI: ${rsi.toFixed(1)}.`
      };
      return belowSMA && rsi > 35 && rsi < 60 ? {
        achieved: true,
        reason: `RSI ${rsi.toFixed(1)} neutral with price below SMA20 — swing short conditions met.`
      } : {
        achieved: false,
        reason: `Swing-trading SELL needs price below SMA20 and RSI between 35–60.`
      };
    case "range_trading":
      {
        ...
        return isRanging ? {
          achieved: true,
          reason: `20-day range is ${rangeWidth.toFixed(1)}% wide and RSI ${rsi.toFixed(1)} is neutral — range-trading conditions met.`
        } : {
          achieved: false,
          reason: `Range-trading requires a well-defined range (< 15% wide) and neutral RSI. Range: ${rangeWidth.toFixed(1)}%, RSI: ${rsi.toFixed(1)}.`
        };
      }
    case "news_based":
      return {
        achieved: true,
        reason: "News/event-based strategy can be applied when a catalyst is present. Ensure a known event is driving this trade."
      };
    case "options_buying":
      return rsi > 60 || rsi < 40 ? {
        achieved: true,
        reason: `RSI ${rsi.toFixed(1)} shows directional conviction — options buying for leveraged directional bet is appropriate.`
      } : {
        achieved: false,
        reason: `Options-buying works best with strong directional conviction (RSI > 60 or < 40). RSI is ${rsi.toFixed(1)} — direction unclear.`
      };
    case "options_selling":
      {
        ...
        return rangeWidth < 12 && rsi > 40 && rsi < 60 ? {
          achieved: true,
          reason: `Price in a ${rangeWidth.toFixed(1)}% range with neutral RSI — low-volatility environment is ideal for options selling.`
        } : {
          achieved: false,
          reason: `Options-selling needs low volatility and range-bound price. Range: ${rangeWidth.toFixed(1)}%, RSI: ${rsi.toFixed(1)}.`
        };
      }
    case "pairs_trading":
      return {
        achieved: false,
        reason: "Pairs-trading requires two correlated symbols. Cannot determine spread conditions with a single asset."
      };
    default:
      return {
        achieved: true,
        reason: "Strategy conditions not specifically defined — proceed with caution."
      };
  }
}
function simulateTrades(strategy, action, closes, highs, lows, sma20, rsi) {
  const trades = [];
  let inTrade = false;
  let entryPrice = 0;
  let entryIdx = 0;
  ...
  const maxHold = 10;
  const exit = (i)=>{
    const ret = action === "BUY" ? (closes[i] - entryPrice) / entryPrice * 100 : (entryPrice - closes[i]) / entryPrice * 100;
    trades.push({
      return: ret,
      entryIdx,
      exitIdx: i
  ...
  for(let i = 20; i < closes.length; i++){
    if (inTrade) {
      ...
      if (action === "BUY" && (ratio <= SL || ratio >= TP) || action === "SELL" && (ratio >= 2 - SL || ratio <= 2 - TP) || holdDays >= maxHold) {
        exit(i);
      }
      continue;
    }
    ...
    switch(strategy){
      ...
      case "momentum":
        ...
        break;
      case "scalping":
        signal = action === "BUY" ? closes[i] < lows[i - 1] * 1.002 && closes[i] > closes[i - 1] * 0.99 : closes[i] > highs[i - 1] * 0.998 && closes[i] < closes[i - 1] * 1.01;
        break;
      case "swing_trading":
        signal = action === "BUY" ? s != null && closes[i] > s && closes[i] < closes[i - 1] && closes[i - 1] < closes[i - 2] && r > 40 && r < 65 : s != null && closes[i] < s && closes[i] > closes[i - 1] && closes[i - 1] > closes[i - 2] && r > 35 && r < 60;
        break;
      case "range_trading":
        signal = action === "BUY" ? closes[i] <= low20 * 1.02 && r < 40 : closes[i] >= high20 * 0.98 && r > 60;
        break;
      case "news_based":
...
function normalizeSymbol(raw) {
  return (raw || "").trim().toUpperCase().replace(/\.(NS|BO)$/i, "").replace(/-USD$/i, "USD").replace(/=X$/i, "");
}
function isIndianSymbol(raw) {
  return /\.(NS|BO)$/i.test(raw);
}
function isCryptoSymbol(raw) {
  return /-USD$|-USDT$|-BTC$/i.test(raw);
}
function isForexSymbol(raw) {
  return /=X$/i.test(raw);
}
...
async function fetchDailyOHLCFromTwelveData(raw, tdKey) {
  try {
...
async function fetchDailyOHLC(symbol, alphaKey) {
  try {
...
async function fetchDailyOHLCFromYahoo(rawSymbol) {
  try {
...
serve(async (req)=>{
  ...
  try {
    ...
    const rawSymbol = body?.symbol ?? "AAPL";
    const strategy = body?.strategy ?? "trend_following";
    const action = (body?.action ?? "BUY") === "SELL" ? "SELL" : "BUY";
    const checkOnly = body?.mode === "check" || body?.check_only === true;
    ...
    const { achieved, reason } = checkStrategyAchieved(strategy, action, latestClose, latestSMA20, latestRSI, high20d, low20d);
    if (checkOnly) {
      return new Response(JSON.stringify({
        symbol: rawSymbol,
        strategy,
