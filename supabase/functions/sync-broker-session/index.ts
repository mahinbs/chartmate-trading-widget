...
/** Compute token expiry: midnight of the next trading day in IST (UTC+5:30) */ function midnightIstExpiry() {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const feedToken = (body.feed_token ?? "").trim() || null;
    const brokerUserId = (body.broker_user_id ?? "").trim() || null;
    if (!broker || !authToken) {
      return new Response(JSON.stringify({
        error: "broker and auth_token are required"
      }), {
        status: 400,
        headers
      });
    ...
    const { error: upsertErr } = await supabase.from("user_trading_integration").upsert({
      user_id: user.id,
    ...
    if (upsertErr) {
      ...
      return new Response(JSON.stringify({
        error: "Failed to save token: " + upsertErr.message
      }), {
        status: 500,
        headers
      });
    ...
    if (OPENALGO_URL && OPENALGO_APP_KEY) {
      ...
      const { data: integration } = await supabase.from("user_trading_integration").select("openalgo_api_key").eq("user_id", user.id).maybeSingle();
      // Get openalgo_username from algo_onboarding → algo_user_assignments
      const { data: assignment } = await supabase.from("algo_user_assignments").select("openalgo_username").eq("user_id", user.id).maybeSingle();
      const openalgoUsername = assignment?.openalgo_username ?? "";
      if (openalgoUsername) {
        try {
