...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    while(Date.now() < deadline){
      ...
      const { data: pending, error: fetchErr } = await supabase.from("pending_conditional_orders").select("id, user_id, strategy_id, symbol, exchange, action, quantity, product, paper_strategy_type, is_paper_trade, created_at, expires_at, deploy_overrides").eq("status", "pending").or(`expires_at.is.null,expires_at.gt.${nowIso}`).order("created_at", {
        ascending: true
