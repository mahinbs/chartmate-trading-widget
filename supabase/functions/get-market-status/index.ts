...
/** Must stay aligned with `allowsLiveEntryExitUI` in StrategyEntrySignalsPanel.tsx */ const MON_FRI_CASH_EXCHANGE_HINTS = [
  'NSE',
  'BSE',
  'BOM',
  'CNX',
...
function isForexSpotSessionOpen(now) {
...
function equityUsesMonFriWeekends(symbol, exchange) {
...
function equityWeekendCheckTimeZone(symbol, exchangeTimezoneName) {
  const sym = symbol.toUpperCase();
...
function isSatOrSunInTimeZone(nowMs, timeZone) {
  try {
...
function isEquityWeekendClosed(symbol, exchange, exchangeTimezoneName, quoteType, nowMs) {
...
/** Single rule-set for whether intraday “Live” entry/exit UI should be allowed. */ function computeLiveSignalsAllowed(r, nowMs) {
...
function attachLiveSignalsAllowed(result, nowMs) {
  result.liveSignalsAllowed = computeLiveSignalsAllowed(result, nowMs);
  return result;
}
serve(async (req)=>{
  ...
  try {
    ...
    const calculateMarketHours = (exchange, timezone, quoteType)=>{
      const now = new Date();
    ...
    const createFallbackStatus = (symbol, exchange, type)=>{
      const exchangeHours = {
        'NYSE': '9:30 AM – 4:00 PM ET',
        'NMS': '9:30 AM – 4:00 PM ET',
      ...
      const exchangeTimezone = exchange === 'LSE' ? 'Europe/London' : exchange === 'NSE' || exchange === 'BSE' ? 'Asia/Kolkata' : exchange === 'HKEX' ? 'Asia/Hong_Kong' : exchange === 'TSE' || exchange === 'JPX' ? 'Asia/Tokyo' : 'America/New_York'; // Default to US
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (quote) {
          ...
          const { symbol: returnSymbol, exchange: yahooExchange = exchange || 'Unknown', marketState = 'CLOSED', quoteType = 'EQUITY' } = quote;
          ...
          const isIndianExchange = INDIAN_EXCHANGES.includes(yahooExchange) || symbol.endsWith('.NS') || symbol.endsWith('.BO');
          const exchangeTimezoneName = isIndianExchange ? 'Asia/Kolkata' : quote.exchangeTimezoneName || 'UTC';
