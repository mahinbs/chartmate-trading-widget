...
async function getOrSetProviderCache(key, ttlMs, fetcher, staleFallbackMs = ttlMs * 5) {
  const now = Date.now();
...
/** Stable labels for Probability Timeline (must match client display). */ function horizonMinutesToLabel(minutes) {
...
function clampProb01(n) {
  return Math.max(0, Math.min(1, n));
}
function averageNewsSentiment(news) {
  if (!news?.length) return 0;
  const v = news.reduce((a, n)=>a + n.sentiment_score, 0) / news.length;
  return Math.max(-1, Math.min(1, v));
}
function providerNewsSkew(pi) {
...
function normalizeProbsToDecimal(p) {
  let u = Number(p.up) || 0;
...
function calibrateDecimalProbsForHorizon(base, tc, newsSent, piSkew, horizonMinutes, horizonIndex) {
  ...
  const skew = Math.max(-0.34, Math.min(0.34, trendNudge + momNudge + volNudge + newsNudge + mrNudge + rsiShort + rsiLong + idxTilt));
...
function forecastDirectionFromProbs(p) {
...
function updatePipelineStep(pipeline, stepName, status, details) {
...
function normalizeCleanSymbol(raw) {
  return raw.replace(/^(NASDAQ|NYSE|NSE|BINANCE|OANDA|SP|DJ|COMEX|NYMEX):/, '').trim().toUpperCase();
}
function resolveCoinGeckoId(raw) {
...
function getAssetRouteInfo(symbol) {
...
function pickRangeForInterval(yahooInterval) {
  const rangeMap = {
    '1m': '5d',
    '5m': '1mo',
    '15m': '1mo',
...
function deriveStockDataFromCandles(candles) {
  if (!candles.length) {
...
async function fetchIntradayCandlesForQuote(yahooSymbol) {
  const intervals = [
    '1m',
    '5m',
    '15m',
...
async function fetchYahooChart(params) {
  ...
  const response = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${yahooSymbol}?interval=${interval}&range=${range}`, {
    headers: {
...
async function fetchAlphaVantageGlobalQuote(symbol) {
  return getOrSetProviderCache(`alpha:quote:${symbol}`, PROVIDER_TTL_MS.alphaQuote, async ()=>{
    const apiKey = getAlphaVantageApiKey();
    if (!apiKey) throw new Error('Alpha Vantage API key missing');
    const response = await fetch(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${encodeURIComponent(symbol)}&apikey=${apiKey}`, {
      signal: AbortSignal.timeout(15000)
    });
...
async function fetchAlphaVantageCandles(symbol, timeframe) {
  return getOrSetProviderCache(`alpha:candles:${symbol}:${timeframe}`, PROVIDER_TTL_MS.alphaCandles, async ()=>{
    const apiKey = getAlphaVantageApiKey();
    if (!apiKey) throw new Error('Alpha Vantage API key missing');
    const interval = mapTimeframeToAlphaInterval(timeframe);
    const url = interval === 'daily' ? `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=${encodeURIComponent(symbol)}&outputsize=full&apikey=${apiKey}` : `https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=${encodeURIComponent(symbol)}&interval=${interval}&outputsize=full&apikey=${apiKey}`;
    ...
    const seriesKey = interval === 'daily' ? 'Time Series (Daily)' : `Time Series (${interval})`;
    const series = data?.[seriesKey];
    if (!series || typeof series !== 'object') throw new Error(`Alpha Vantage series missing for ${symbol}`);
    const candles = Object.entries(series).map(([ts, row])=>({
      ...
      })).filter((c)=>c.close > 0).sort((a, b)=>a.timestamp - b.timestamp);
    return candles;
  });
}
async function fetchCoinGeckoMarketData(coinId) {
  return getOrSetProviderCache(`cg:market:${coinId}`, PROVIDER_TTL_MS.coingeckoMarket, async ()=>{
    const apiKey = getCoinGeckoApiKey();
    const headers = apiKey ? {
      'x-cg-demo-api-key': apiKey
    } : {};
    const marketResponse = await fetch(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${encodeURIComponent(coinId)}&price_change_percentage=24h`, {
      headers,
      signal: AbortSignal.timeout(15000)
    });
    ...
    const chartResponse = await fetch(`https://api.coingecko.com/api/v3/coins/${encodeURIComponent(coinId)}/market_chart?vs_currency=usd&days=30&interval=daily`, {
      headers,
      signal: AbortSignal.timeout(15000)
    });
    ...
    const volumeByTs = new Map(volumes.map((v)=>[
        v[0],
        Number(v[1] || 0)
      ]));
    const candles = prices.map(([ts, close], idx)=>{
...
async function fetchExternalMarketSnapshot(symbol, stockData, fundamentals) {
...
async function fetchAlphaVantageRSI(symbol, timeframe) {
  try {
    return await getOrSetProviderCache(`alpha:rsi:${symbol}:${timeframe}`, PROVIDER_TTL_MS.alphaIndicators, async ()=>{
      const apiKey = getAlphaVantageApiKey();
      if (!apiKey) return null;
      const interval = mapTimeframeToAlphaInterval(timeframe);
      const adjustedInterval = interval === 'daily' ? 'daily' : interval;
      const response = await fetch(`https://www.alphavantage.co/query?function=RSI&symbol=${encodeURIComponent(symbol)}&interval=${adjustedInterval}&time_period=14&series_type=close&apikey=${apiKey}`, {
        signal: AbortSignal.timeout(15000)
      });
...
async function fetchAlphaVantageBollingerBands(symbol, timeframe) {
  try {
    return await getOrSetProviderCache(`alpha:bbands:${symbol}:${timeframe}`, PROVIDER_TTL_MS.alphaIndicators, async ()=>{
      const apiKey = getAlphaVantageApiKey();
      if (!apiKey) return null;
      const interval = mapTimeframeToAlphaInterval(timeframe);
      const adjustedInterval = interval === 'daily' ? 'daily' : interval;
      const response = await fetch(`https://www.alphavantage.co/query?function=BBANDS&symbol=${encodeURIComponent(symbol)}&interval=${adjustedInterval}&time_period=20&series_type=close&nbdevup=2&nbdevdn=2&apikey=${apiKey}`, {
        signal: AbortSignal.timeout(15000)
      });
      if (!response.ok) return null;
      const data = await response.json();
...
async function fetchAlphaVantageNewsSentimentScore(symbol) {
  try {
    return await getOrSetProviderCache(`alpha:newsSent:${symbol}`, PROVIDER_TTL_MS.alphaNewsSentiment, async ()=>{
      const apiKey = getAlphaVantageApiKey();
      if (!apiKey) return null;
      const response = await fetch(`https://www.alphavantage.co/query?function=NEWS_SENTIMENT&tickers=${encodeURIComponent(symbol)}&apikey=${apiKey}&limit=50`, {
        signal: AbortSignal.timeout(15000)
      });
      if (!response.ok) return null;
      const data = await response.json();
      ...
      const scored = feed.map((item)=>Number(item?.overall_sentiment_score)).filter((v)=>Number.isFinite(v));
...
async function fetchCoinGeckoDominanceAndTrending() {
  try {
    return await getOrSetProviderCache('cg:global:dominance-trending', PROVIDER_TTL_MS.coingeckoGlobal, async ()=>{
      const apiKey = getCoinGeckoApiKey();
      const headers = apiKey ? {
        'x-cg-demo-api-key': apiKey
      } : {};
      ...
      const trendingCoins = Array.isArray(trendingJson?.coins) ? trendingJson.coins.slice(0, 10).map((c)=>c?.item?.symbol).filter(Boolean) : [];
      return {
...
async function fetchTwelveDataIntelligence(tdSymbol, interval, computedBB) {
  ...
  const avgVol10d = statJson?.statistics?.stock_statistics?.average_volume_10d_calc ? parseFloat(statJson.statistics.stock_statistics.average_volume_10d_calc) : null;
  const avgVol3m = statJson?.statistics?.stock_statistics?.average_volume_3m_calc ? parseFloat(statJson.statistics.stock_statistics.average_volume_3m_calc) : null;
  const sharesFloat = statJson?.statistics?.stock_statistics?.shares_float ? parseFloat(statJson.statistics.stock_statistics.shares_float) : null;
  const notes = [
...
function toTwelveDataSymbol(symbol, assetType) {
...
async function buildProviderIntelligence(symbol, timeframe, stockData, candles, technicalContext) {
  const route = getAssetRouteInfo(symbol);
  const computedBB = {
  ...
  if (route.isUsStock || route.assetType === 'crypto' || route.assetType === 'forex') {
    ...
    const tdInterval = timeframe === '15m' ? '15min' : timeframe === '30m' ? '30min' : timeframe === '1h' ? '1h' : timeframe === '4h' ? '4h' : '1day';
    try {
      ...
      if (route.isUsStock) {
        try {
          ...
          intelligence.momentum = {
            ...
            volume24h: candles[candles.length - 1]?.volume ?? null,
            rsi,
            notes: 'TwelveData unavailable; using Alpha Vantage'
          };
          intelligence.volatility = {
            source: 'Alpha Vantage (fallback)',
            bollingerBands: bbands ?? computedBB,
            optionGreeks: null
          };
          intelligence.sentiment = {
            source: 'Alpha Vantage (fallback)',
      ...
      if (route.assetType === 'crypto') {
        try {
          ...
          intelligence.momentum = {
            ...
            rsi: Number.isFinite(rsiFromPrices) ? Number(rsiFromPrices) : null,
            notes: 'TwelveData unavailable; using CoinGecko'
          };
          intelligence.sentiment = {
            source: 'CoinGecko (fallback)',
            btcDominance: dominanceAndTrending.btcDominance,
...
async function fetchTwelveDataCandles(tdSymbol, interval, outputsize = 100) {
  const tdKey = Deno.env.get('TWELVE_DATA_API_KEY');
  if (!tdKey) throw new Error('TWELVE_DATA_API_KEY not set');
...
function mapTimeframeToTDInterval(timeframe) {
  const map = {
    '15m': '15min',
    '30m': '30min',
    '1h': '1h',
...
async function fetchRealStockData(symbol) {
  ...
  if (route.isUsStock || route.assetType === 'crypto' || route.assetType === 'forex') {
    ...
    if (tdKey) {
      const tdSymbol = toTwelveDataSymbol(symbol, route.assetType);
      try {
        const res = await fetch(`https://api.twelvedata.com/quote?symbol=${encodeURIComponent(tdSymbol)}&apikey=${tdKey}`, {
...
function getEnhancedFallbackData(symbol) {
...
async function fetchAlphaVantageNews(symbol) {
  try {
    ...
    const response = await fetch(`https://www.alphavantage.co/query?function=NEWS_SENTIMENT&tickers=${symbol}&apikey=${alphaVantageKey}&limit=20`, {
      signal: AbortSignal.timeout(10000)
    });
    if (!response.ok) {
      return [];
...
async function fetchFullYearHistory(symbol) {
  const route = getAssetRouteInfo(symbol);
  ...
  try {
    if (route.isUsStock) {
      ...
      const yearTrend = yearChange > 15 ? 'strong_uptrend' : yearChange > 5 ? 'uptrend' : yearChange > -5 ? 'sideways' : yearChange > -15 ? 'downtrend' : 'strong_downtrend';
    ...
    const response = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${yahooSymbol}?interval=1d&range=1y`, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      },
      signal: AbortSignal.timeout(15000)
...
async function fetchFundamentals(symbol) {
  ...
  try {
    ...
    if (route.isUsStock) {
      const apiKey = getAlphaVantageApiKey();
      if (apiKey) {
        const response = await fetch(`https://www.alphavantage.co/query?function=OVERVIEW&symbol=${encodeURIComponent(route.cleanSymbol)}&apikey=${apiKey}`, {
          signal: AbortSignal.timeout(15000)
        });
        if (response.ok) {
    ...
    const response = await fetch(`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${yahooSymbol}?modules=defaultKeyStatistics,financialData,summaryDetail,earnings`, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      },
      signal: AbortSignal.timeout(15000)
...
async function fetchEarningsHistory(symbol) {
  ...
  try {
    ...
    if (route.isUsStock) {
      const apiKey = getAlphaVantageApiKey();
      if (apiKey) {
        const response = await fetch(`https://www.alphavantage.co/query?function=EARNINGS&symbol=${encodeURIComponent(route.cleanSymbol)}&apikey=${apiKey}`, {
          signal: AbortSignal.timeout(15000)
        });
        if (response.ok) {
          const data = await response.json();
          const q = Array.isArray(data?.quarterlyEarnings) ? data.quarterlyEarnings : [];
          return q.slice(0, 8).map((e)=>({
    ...
    const response = await fetch(`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${yahooSymbol}?modules=earnings,earningsHistory`, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      },
      signal: AbortSignal.timeout(15000)
...
async function fetchYahooFinanceNews(symbol) {
  try {
    ...
    const response = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?interval=1d&range=5d&includePrePost=false&events=news`, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      },
      signal: AbortSignal.timeout(15000)
    ...
    if (data.chart?.result?.[0]?.news) {
      return data.chart.result[0].news.map((item)=>{
        const link = typeof item.link === 'string' ? item.link : typeof item.clickThroughUrl === 'string' ? item.clickThroughUrl : undefined;
...
// MarketWatch News
async function fetchMarketWatchNews(symbol) {
  try {
    // MarketWatch RSS feed approach
    const response = await fetch(`https://www.marketwatch.com/investing/stock/${symbol.toLowerCase()}/news`, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      },
      signal: AbortSignal.timeout(15000)
    ...
    if (headlineMatches) {
      return headlineMatches.slice(0, 5).map((match)=>{
...
function removeDuplicateNews(newsItems) {
  ...
  return newsItems.filter((item)=>{
    const key = item.url && item.url.startsWith("http") ? item.url : `${item.headline.toLowerCase().slice(0, 50)}-${item.source}`;
    if (seen.has(key)) {
      return false;
    }
    seen.add(key);
...
function computeEnhancedTechnicalContext(candles, stockData) {
  if (!candles.length) {
    return {
      candles,
      indicators: {
...
function findPriceClusters(prices, threshold) {
  const clusters = [];
...
function calculateVolumeTrend(volumes, prices) {
  ...
  return volumePriceCorrelation / (10 * Math.sqrt(recentVolumes.reduce((sum, v)=>sum + Math.pow(v - avgVolume, 2), 0) / 10) * Math.sqrt(recentPrices.reduce((sum, p)=>sum + Math.pow(p - avgPrice, 2), 0) / 10));
}
...
function calculateMeanReversionSignal(prices, sma, atr) {
  if (prices.length === 0) return 0;
...
function calculateRiskGrade(volatility, leverage = 1, riskFlags, confidence) {
  let riskScore = 0;
...
function deriveActionSignal(direction, bias, confidence) {
  let action;
  let urgency;
  // Determine action
...
function calculateExpectedROI(expectedReturnBp, confidence, volatility) {
...
async function generateEnhancedGeminiAnalysis(symbol, stockData, technicalContext, newsData, investment, horizons, focusTimeframe, userContext, enhancedData) {
...
EARNINGS HISTORY:
${enhancedData?.earningsHistory && enhancedData.earningsHistory.length > 0 ? enhancedData.earningsHistory.slice(0, 4).map((e)=>`- ${e.date}: Actual: $${e.actual?.toFixed(2) || 'N/A'}, Estimate: $${e.estimate?.toFixed(2) || 'N/A'} (${e.actual && e.estimate ? ((e.actual - e.estimate) / e.estimate * 100).toFixed(1) + '% ' + (e.actual > e.estimate ? 'beat' : 'miss') : ''})`).join('\n') : '- No recent earnings data'}
...
- Interpretation: ${Math.abs(spyCorrelation.coefficient) > 0.7 ? 'Stock highly correlated with market - macro factors dominate' : Math.abs(spyCorrelation.coefficient) > 0.4 ? 'Stock moderately tied to market - mix of macro & company-specific' : 'Stock moves independently - focus on company-specific factors'}
- Risk Insight: ${spyCorrelation.coefficient < 0 ? '⚠️ NEGATIVE correlation - potential hedge/defensive play' : spyCorrelation.coefficient > 0.8 ? '⚠️ BETA risk - will amplify market moves' : '✓ Moderate market sensitivity'}` : '- Correlation data not available'}
...
function generateEnsemblePrediction(symbol, stockData, technicalContext, newsData, horizons) {
  const forecasts = horizons.map((horizon)=>{
    ...
    // Combined score
    const combinedScore = technicalScore * weights.technical + meanReversionScore * weights.meanReversion + volumePriceScore * weights.volumePrice + sentimentScore * weights.sentiment;
    let direction = "sideways";
    let confidence = 50;
    // Calculate expected returns based on volatility and momentum
    const volatilityMultiplier = technicalContext.volatilityRegime === 'high' ? 1.5 : technicalContext.volatilityRegime === 'elevated' ? 1.2 : 1.0;
  ...
  return {
    ...
    forecasts,
    support_resistance: {
      supports: technicalContext.supportLevels.map((level)=>({
          level,
          strength: 0.7
        })),
      resistances: technicalContext.resistanceLevels.map((level)=>({
          level,
          strength: 0.7
        }))
    },
    positioning_guidance: {
      bias: forecasts[0]?.direction === 'up' ? 'long' : forecasts[0]?.direction === 'down' ? 'short' : 'flat',
      notes: generatePositioningNotes(technicalContext, forecasts[0])
    }
  };
}
function sanitizeAndCalibrateForecast(forecast, horizons, technicalContext, newsData, providerIntelligence, symbol, stockData) {
  ...
  while(forecast.forecasts.length < horizons.length){
    ...
    const patch = generateEnsemblePrediction(symbol, stockData, technicalContext, newsData, [
      h
    ]);
  ...
  for(let i = 0; i < horizons.length; i++){
    ...
    const calibrated = calibrateDecimalProbsForHorizon(row.probabilities || {
      up: 0.33,
      down: 0.33,
      sideways: 0.34
    }, technicalContext, newsSent, piSkew, h, i);
    row.probabilities = calibrated;
    row.direction = forecastDirectionFromProbs(calibrated);
  }
}
function buildPublicRationale(forecast) {
  const ext = forecast.executive_rationale?.trim();
...
function calculateTechnicalScore(context1) {
  let score = 0;
  // Trend strength contribution
  score += context1.trendStrength * 0.4;
  // RSI contribution
  if (context1.indicators.rsi < 30) score += 0.3; // Oversold
  else if (context1.indicators.rsi > 70) score -= 0.3; // Overbought
  else score += (50 - context1.indicators.rsi) / 100; // Neutral zone
  // MACD contribution
  if (context1.indicators.macd > context1.indicators.macdSignal) score += 0.2;
  else score -= 0.2;
  // Bollinger Bands contribution
  const bbPosition = (context1.indicators.bbUpper - context1.indicators.bbLower) / context1.indicators.bbMiddle;
  if (bbPosition < 0.02) score += 0.1; // Squeeze potential
  return Math.max(-1, Math.min(1, score));
}
function calculateMeanReversionScore(context1) {
  return -context1.meanReversionSignal; // Inverse relationship
}
function calculateVolumePriceScore(context1) {
  let score = 0;
  // Volume confirmation
  score += context1.volumeConfirmation * 0.5;
  // Volume trend
  if (context1.patterns.includes('volume_trend_bullish')) score += 0.3;
  if (context1.patterns.includes('volume_trend_bearish')) score -= 0.3;
  return Math.max(-1, Math.min(1, score));
}
...
function getKeyDrivers(context1, newsData) {
  const drivers = [];
  if (context1.trendStrength > 0.5) drivers.push('strong_trend');
  if (context1.momentum > 0.02) drivers.push('momentum');
  if (context1.volumeConfirmation > 0) drivers.push('volume_confirmation');
  if (context1.patterns.includes('bb_squeeze')) drivers.push('volatility_breakout_potential');
  if (Math.abs(context1.meanReversionSignal) > 0.7) drivers.push('mean_reversion');
...
function getInvalidConditions(horizon, context1) {
  const conditions = [];
  if (horizon >= 1440 && context1.volatilityRegime === 'extreme') {
    conditions.push('extreme_volatility_unsuitable_for_daily');
  }
  if (context1.patterns.includes('bb_squeeze')) {
    conditions.push('volatility_breakout_imminent');
  }
  return conditions;
}
function generatePositioningNotes(context1, forecast) {
  let notes = '';
  if (context1.trendStrength > 0.5) {
    notes += 'Strong trend following recommended. ';
  } else if (Math.abs(context1.meanReversionSignal) > 0.7) {
    notes += 'Mean reversion setup detected. ';
  }
  if (context1.volumeConfirmation === 0) {
    notes += 'Wait for volume confirmation. ';
  }
  if (context1.volatilityRegime === 'high') {
...
function getPredictionModel(symbol) {
...
function updateModelWeights(model, recentAccuracy) {
...
function generateEnhancedEnsemblePrediction(symbol, stockData, technicalContext, newsData, horizons) {
  ...
  const forecasts = horizons.map((horizon)=>{
    ...
    const sentimentScore = calculateSentimentScore(newsData);
    // Use learned weights
    const combinedScore = technicalScore * model.modelWeights.technical + meanReversionScore * model.modelWeights.meanReversion + volumePriceScore * model.modelWeights.volumePrice + sentimentScore * model.modelWeights.sentiment;
    const volatilityMultiplier = technicalContext.volatilityRegime === 'high' ? 1.5 : technicalContext.volatilityRegime === 'elevated' ? 1.2 : 1.0;
    const horizonScale = 0.88 + Math.log(1 + horizon) / Math.log(10081) * 0.32;
    const scoreForHorizon = Math.max(-1, Math.min(1, combinedScore * horizonScale));
    let direction = "sideways";
...
function detectMarketRegime(candles, technicalContext) {
  const closePrices = candles.map((c)=>c.close);
  const volumes = candles.map((c)=>c.volume);
  ...
  return {
    ...
    volatility: technicalContext.volatilityRegime,
...
function calculateAdvancedTrendStrength(prices) {
...
function calculateMomentumIndex(prices, volumes) {
...
function calculateDynamicSupportResistance(prices) {
...
function calculateRegimeConfidence(context1) {
  let confidence = 0.5; // Base confidence
  // Pattern confidence
  if (context1.patterns.includes('strong_trend_bullish') || context1.patterns.includes('strong_trend_bearish')) {
    confidence += 0.2;
  }
  // Volume confirmation
  if (context1.volumeConfirmation > 0) {
  ...
  // RSI confidence
  if (context1.indicators.rsi < 30 || context1.indicators.rsi > 70) {
    confidence += 0.1;
  }
  // MACD confidence
  if (Math.abs(context1.indicators.macd) > Math.abs(context1.indicators.macdSignal)) {
    confidence += 0.1;
  }
  return Math.min(1, confidence);
}
/** Build human-readable key factor sentences from live technical context */ function buildReadableKeyFactors(context1, regime1) {
  const factors = [];
  const ind = context1.indicators;
  ...
  if (context1.volumeProfile === 'high') factors.push('Above-average volume confirming price move strength');
  else if (context1.volumeProfile === 'low') factors.push('Below-average volume — move may lack conviction');
  if (regime1.type === 'trending') factors.push(`Trending market regime (strength: ${(regime1.strength * 100).toFixed(0)}%) supports directional bias`);
  else if (regime1.type === 'volatile') factors.push('Volatile regime detected — wider price swings likely');
...
/** Build human-readable risk factor sentences from live technical context */ function buildReadableRiskFactors(context1, regime1) {
  const risks = [];
  if (regime1.type === 'volatile') risks.push('High volatility regime — position sizing caution advised');
  if (context1.volatilityState === 'high') risks.push('ATR indicates elevated intraday volatility');
  if (context1.indicators.rsi > 65) risks.push('Overbought conditions increase pullback risk');
  if (context1.indicators.rsi < 35) risks.push('Oversold conditions may face continued selling pressure');
  if (context1.volumeProfile === 'low') risks.push('Low volume — signal reliability reduced');
  risks.push('Macro events or news may override technical signals');
  return risks.slice(0, 3);
}
// GODLY PLAN: Quantum Ensemble Prediction
function generateQuantumPrediction(symbol, stockData, technicalContext, newsData, marketRegime, horizons) {
...
function generateTechnicalPrediction(context1, regime1) {
  let direction = 'sideways';
  let confidence = 0.5;
  // Advanced pattern recognition
  if (context1.patterns.includes('strong_trend_bullish') && regime1.momentum === 'bullish') {
    direction = 'up';
    confidence = 0.8;
  } else if (context1.patterns.includes('strong_trend_bearish') && regime1.momentum === 'bearish') {
    direction = 'down';
    confidence = 0.8;
  } else if (context1.patterns.includes('mean_reversion_bullish') && context1.meanReversionSignal > 0.7) {
    direction = 'up';
    confidence = 0.7;
  } else if (context1.patterns.includes('mean_reversion_bearish') && context1.meanReversionSignal < -0.7) {
    direction = 'down';
    confidence = 0.7;
  }
  return {
...
function generateStatisticalPrediction(context1, stockData) {
  // Statistical analysis based on historical patterns
  const rsiSignal = context1.indicators.rsi < 30 ? 'up' : context1.indicators.rsi > 70 ? 'down' : 'sideways';
  const macdSignal = context1.indicators.macd > context1.indicators.macdSignal ? 'up' : 'down';
  let direction = 'sideways';
  let confidence = 0.5;
...
function generateSentimentPrediction(newsData, context1) {
...
function generateMomentumPrediction(context1, stockData) {
  let direction = 'sideways';
  let confidence = 0.5;
  if (context1.momentum > 0.02) {
    direction = 'up';
    confidence = Math.min(0.8, 0.5 + context1.momentum * 10);
  } else if (context1.momentum < -0.02) {
    direction = 'down';
    confidence = Math.min(0.8, 0.5 + Math.abs(context1.momentum) * 10);
  }
  return {
    direction,
    confidence
...
function generateMeanReversionPrediction(context1, stockData) {
  let direction = 'sideways';
  let confidence = 0.5;
  if (context1.meanReversionSignal > 0.7) {
    direction = 'up';
    confidence = Math.min(0.8, 0.5 + context1.meanReversionSignal * 0.3);
  } else if (context1.meanReversionSignal < -0.7) {
    direction = 'down';
    confidence = Math.min(0.8, 0.5 + Math.abs(context1.meanReversionSignal) * 0.3);
  }
  return {
    direction,
    confidence
...
function generateVolatilityPrediction(context1, regime1) {
  // Volatility-based predictions
  let direction = 'sideways';
  let confidence = 0.5;
  if (regime1.volatility === 'extreme') {
    // High volatility often leads to mean reversion
    if (context1.meanReversionSignal > 0.5) {
      direction = 'up';
      confidence = 0.6;
    } else if (context1.meanReversionSignal < -0.5) {
      direction = 'down';
      confidence = 0.6;
    }
  }
...
function getQuantumWeights(regime1) {
  const baseWeights = {
    technical: 0.25,
    statistical: 0.20,
    sentiment: 0.15,
  ...
  if (regime1.type === 'trending') {
    baseWeights.momentum += 0.1;
    baseWeights.meanReversion -= 0.1;
  } else if (regime1.type === 'ranging') {
    baseWeights.meanReversion += 0.1;
    baseWeights.momentum -= 0.1;
  } else if (regime1.type === 'volatile') {
...
function combinePredictions(models, weights, horizons) {
...
function calculateSuccessProbability(predictions, regime1) {
  ...
  if (regime1.type === 'trending' && regime1.strength > 0.7) {
    baseProbability += 0.1;
  } else if (regime1.type === 'volatile') {
    baseProbability -= 0.1;
  }
  // Adjust for regime confidence
  baseProbability *= regime1.confidence;
  return Math.min(0.95, Math.max(0.05, baseProbability));
}
// GODLY PLAN: Alternative Scenarios
function generateAlternativeScenarios(predictions, regime1) {
  const scenarios = [];
...
function calculateRiskScore(predictions, regime1) {
  let riskScore = 0.5; // Base risk
  // Volatility risk
  if (regime1.volatility === 'extreme') riskScore += 0.3;
  else if (regime1.volatility === 'elevated') riskScore += 0.2;
  // Regime stability risk
  if (regime1.confidence < 0.7) riskScore += 0.2;
...
serve(async (req)=>{
  ...
  try {
    ...
    const { symbol, investment, timeframe, focusTimeframe, horizons = [
      60,
      240,
      1440,
      10080
    ], riskTolerance, tradingStyle, investmentGoal, stopLossPercentage, targetProfitPercentage, leverage, marginType } = requestBody;
    ...
    if (!symbol || !investment || !timeframe) {
      ...
      return new Response(JSON.stringify({
        error: 'Missing required fields: symbol, investment, timeframe'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    updatePipelineStep(pipeline, 'enhanced_data_analysis', 'completed', `Year: ${fullYearData.yearTrend}, P/E: ${fundamentals.peRatio?.toFixed(2) || 'N/A'}, ${earningsHistory.length} earnings`);
    ...
    try {
      if (!routeInfo.isUsStock) {
        ...
        if (spyData.candles.length > 10 && historicalData.candles.length > 10) {
          ...
          const stockReturns = historicalData.candles.slice(0, minLength).map((c, i)=>i === 0 ? 0 : (c.close - historicalData.candles[i - 1].close) / historicalData.candles[i - 1].close);
          const spyReturns = spyData.candles.slice(0, minLength).map((c, i)=>i === 0 ? 0 : (c.close - spyData.candles[i - 1].close) / spyData.candles[i - 1].close);
          ...
          console.log(`📊 SPY Correlation: ${(correlation * 100).toFixed(1)}% (${spyCorrelation1.relationship})`);
        }
        updatePipelineStep(pipeline, 'market_correlation', 'completed', spyCorrelation1 ? `Correlation: ${(spyCorrelation1.coefficient * 100).toFixed(0)}%` : 'Calculated');
      }
    } catch (error) {
    ...
    const providerIntelligence = await buildProviderIntelligence(symbol, timeframe, stockData, historicalData.candles, technicalContext);
    updatePipelineStep(pipeline, 'technical_indicators', 'completed', `RSI: ${technicalContext.indicators.rsi.toFixed(1)}, Trend: ${technicalContext.trendDirection}`);
    // 🌟 GODLY PLAN: Market Regime Detection 🌟
    ...
    updatePipelineStep(pipeline, 'market_regime_detection', 'completed', `Regime: ${marketRegime.type} (${(marketRegime.strength * 100).toFixed(0)}% strength)`);
    ...
    try {
      ...
      geminiForecast = await generateEnhancedGeminiAnalysis(symbol, stockData, technicalContext, newsData, investment, horizons, focusTimeframe ?? timeframe, userContext, {
      ...
      try {
        const quantumPrediction = generateQuantumPrediction(symbol, stockData, technicalContext, newsData, marketRegime, horizons);
        geminiForecast = {
          ...
          support_resistance: {
            supports: [
              {
                level: marketRegime.support,
                strength: 0.9
              }
            ],
            resistances: [
              {
                level: marketRegime.resistance,
                strength: 0.9
              }
            ]
          },
          positioning_guidance: {
            bias: quantumPrediction.predictions.shortTerm.direction === 'up' ? 'long' : quantumPrediction.predictions.shortTerm.direction === 'down' ? 'short' : 'flat',
        ...
        geminiForecast = generateEnhancedEnsemblePrediction(symbol, stockData, technicalContext, newsData, horizons);
    ...
    sanitizeAndCalibrateForecast(geminiForecast, horizons, technicalContext, newsData, providerIntelligence, symbol, stockData);
    updatePipelineStep(pipeline, 'multi_horizon_forecast', 'completed', `${geminiForecast.forecasts.length} horizons analyzed (${predictionSource})`);
    ...
    const probabilityScore = typeof primaryForecast?.probabilities?.up === 'number' && typeof primaryForecast?.probabilities?.down === 'number' && typeof primaryForecast?.probabilities?.sideways === 'number' ? Math.max(Math.min(1, primaryForecast.probabilities.up || 0), Math.min(1, primaryForecast.probabilities.down || 0), Math.min(1, primaryForecast.probabilities.sideways || 0)) : Math.max(0, Math.min(1, (geminiForecast?.deep_analysis?.success_probability || 0) / 100));
    ...
    const actionSignal = geminiReturnedSignal === "BUY" || geminiReturnedSignal === "SELL" ? geminiForecast.action_signal : deriveActionSignal(primaryForecast.direction, geminiForecast.positioning_guidance.bias, primaryForecast.confidence);
    // Calculate risk grade
    const riskGrade = calculateRiskGrade(volatilityPercent, leverage, primaryForecast.risk_flags, primaryForecast.confidence);
    // Calculate expected ROI ranges
    const expectedROI = calculateExpectedROI(primaryForecast.expected_return_bp, primaryForecast.confidence, volatilityPercent);
    ...
    const sharesQuantity = isCryptoAsset ? parseFloat(rawQty.toFixed(6)) // up to 6 decimal places for crypto
    ...
    const result = {
      ...
      recommendation: geminiForecast?.positioning_guidance?.bias === "long" ? "bullish" : geminiForecast?.positioning_guidance?.bias === "short" ? "bearish" : "neutral",
      ...
      expectedMove: {
        ...
        direction: geminiForecast?.forecasts?.[0]?.direction || "flat",
      ...
      volumeData: {
        ...
        avgVolume: (()=>{
          const vols = technicalContext.candles.slice(-20).map((c)=>c.volume);
