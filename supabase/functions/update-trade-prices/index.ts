...
// Calculate P&L
function calculatePnL(entryPrice, currentPrice, shares, leverage = 1.0) {
...
function calculateTrailingStop(entryPrice, currentPrice, highestPrice, stopLossPercentage, action) {
...
function checkExitConditions(trade, currentPrice, highestPrice) {
  ...
  if (trade.action === 'BUY' && trade.stop_loss_percentage) {
    const trailingStop = calculateTrailingStop(trade.entry_price, currentPrice, highestPrice, trade.stop_loss_percentage, trade.action);
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: activeTrades, error: fetchError } = await supabaseClient.from('active_trades').select('*').in('status', [
      'active',
      'monitoring',
      'exit_zone'
    ]);
    ...
    for (const trade of activeTrades){
      try {
        ...
        // Calculate P&L
        const { pnl, pnlPercentage } = calculatePnL(trade.entry_price, currentPrice, trade.shares, trade.leverage || 1.0);
        // Track highest price for trailing stop (fetch from history or current)
        const { data: priceHistory } = await supabaseClient.from('trade_updates').select('price').eq('trade_id', trade.id).order('price', {
          ascending: false
        }).limit(1).single();
        const highestPrice = Math.max(trade.entry_price, currentPrice, priceHistory?.price || 0);
        // Check exit conditions (with trailing stop support)
        const { shouldExit, reason, newStopLoss } = checkExitConditions(trade, currentPrice, highestPrice);
        // Determine status
        let newStatus = trade.status;
        if (shouldExit) {
          newStatus = reason === 'stop_loss_triggered' ? 'stopped_out' : reason === 'target_hit' ? 'target_hit' : 'completed';
        } else {
        ...
        if (shouldExit) {
          ...
          if (trade.broker_order_id) {
            try {
              ...
              const { data: integration } = await supabaseClient.from('user_trading_integration').select('openalgo_api_key').eq('user_id', trade.user_id).eq('is_active', true).maybeSingle();
              ...
              if (OPENALGO_URL && openalgoApiKey) {
                ...
                const exitOrderId = exitData?.orderid ?? exitData?.order_id;
                if (exitOrderId) {
        ...
        const { error: updateError } = await supabaseClient.from('active_trades').update(updateData).eq('id', trade.id);
        if (updateError) {
        ...
        await supabaseClient.from('trade_updates').insert({
          trade_id: trade.id,
          price: currentPrice,
          pnl: pnl,
        ...
        if (shouldExit) {
          ...
          await supabaseClient.from('trade_notifications').insert({
            trade_id: trade.id,
            user_id: trade.user_id,
            type: reason === 'stop_loss_triggered' ? 'stop_loss_triggered' : reason === 'target_hit' ? 'target_hit' : 'holding_period_ending',
          ...
          // Mark alert as sent
          await supabaseClient.from('active_trades').update({
            exit_zone_alert_sent: true
          }).eq('id', trade.id);
        }
        updated++;
