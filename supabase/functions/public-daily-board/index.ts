...
serve(async (req)=>{
  ...
  try {
    ...
    let targetDate = date;
    if (!targetDate) {
      const { data: latestRow, error: latestError } = await supabaseClient.from("daily_predictions_board").select("generated_for_date").eq("is_active", true).order("generated_for_date", {
        ascending: false
      }).limit(1).maybeSingle();
    ...
    const { data, error } = await supabaseClient.from("daily_predictions_board").select("*").eq("generated_for_date", targetDate).eq("is_active", true).order("sort_order", {
      ascending: true
    });
