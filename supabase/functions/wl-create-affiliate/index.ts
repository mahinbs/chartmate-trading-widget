...
/** Assert the caller is a white-label tenant admin */ async function assertWhitelabelAdmin(userId, tenantId, supabaseClient) {
  const { data, error } = await supabaseClient.from("white_label_tenant_users").select("role, status").eq("tenant_id", tenantId).eq("user_id", userId).maybeSingle();
  if (error) throw new Error(`Failed to validate WL membership: ${error.message}`);
  if (!data || data.role !== "admin") throw new Error("Forbidden: white-label admin access required");
  if (data.status !== "active") throw new Error("Your white-label account is suspended");
}
serve(async (req)=>{
  ...
  try {
    ...
    const { tenant_id, code, name, email, commission_percent } = body;
    if (!tenant_id) {
      return new Response(JSON.stringify({
