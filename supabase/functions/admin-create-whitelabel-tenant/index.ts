...
async function assertSuperAdmin(userId, supabaseClient) {
  const { data: roleRow, error } = await supabaseClient.from("user_roles").select("role").eq("user_id", userId).maybeSingle();
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const subscriptionPlan = (body.subscription_plan ?? "1_year").trim();
    const brandLogoUrl = (body.brand_logo_url ?? "").trim() || null;
    const brandPrimaryColor = (body.brand_primary_color ?? "#6366f1").trim();
    const brandTagline = (body.brand_tagline ?? "").trim() || null;
    if (!ownerEmail || !slug || !brandName || !startsOn || !endsOn) {
      return new Response(JSON.stringify({
        error: "owner_email, slug, brand_name, starts_on, and ends_on are required"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: existingTenant } = await supabaseClient.from("white_label_tenants").select("id").eq("slug", slug).maybeSingle();
    if (existingTenant) {
      return new Response(JSON.stringify({
    ...
    const { error: roleErr } = await supabaseClient.from("user_roles").upsert({
      user_id: ownerUserId,
      role: "admin"
    }, {
      onConflict: "user_id"
    ...
    const { data: tenant, error: tenantErr } = await supabaseClient.from("white_label_tenants").insert({
      slug,
    ...
    const { error: membershipErr } = await supabaseClient.from("white_label_tenant_users").upsert({
      tenant_id: tenant.id,
      user_id: ownerUserId,
      role: "admin",
      status: "active"
