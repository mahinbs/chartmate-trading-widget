...
serve(async (req)=>{
  ...
  try {
    ...
    if (userIds.length > 0) {
      const { data: rolesData, error: rolesError } = await supabaseClient.from("user_roles").select("user_id, role").in("user_id", userIds);
      if (rolesError) throw new Error(`Failed to fetch roles: ${rolesError.message}`);
      rolesByUserId = new Map(rolesData.map((r)=>[
          r.user_id,
          r.role
        ]));
    }
