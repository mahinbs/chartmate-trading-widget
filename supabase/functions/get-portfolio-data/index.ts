...
async function oaPost(endpoint, apiKey) {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: integration } = await supabase.from("user_trading_integration").select("openalgo_api_key, broker, is_active, token_expires_at").eq("user_id", user.id).eq("is_active", true).maybeSingle();
    const apiKey = integration?.openalgo_api_key ?? "";
    const broker = integration?.broker ?? null;
    const tokenExpiry = integration?.token_expires_at ?? null;
    if (!apiKey.trim()) {
      return new Response(JSON.stringify({
    ...
    const [fundsResult, positionsResult, holdingsResult, ordersResult, tradeBookResult, openPosResult] = await Promise.allSettled([
      ...
      oaPost("/api/v1/tradebook", apiKey),
      oaPost("/api/v1/openposition", apiKey)
    ]);
    const fulfilled = (r)=>r.status === "fulfilled" ? r.value : null;
    const extractError = (r)=>r.status === "rejected" ? r.reason?.message ?? "Error" : null;
    ...
    return new Response(JSON.stringify({
      ...
      tradebook: Array.isArray(tradebook) ? tradebook : [],
      open_positions: Array.isArray(openPos) ? openPos : [],
      errors: {
