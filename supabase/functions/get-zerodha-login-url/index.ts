...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    try {
      ...
      return new Response(JSON.stringify({
        error: "Invalid JSON body; return_url required"
      }), {
        status: 400,
        headers
      });
    ...
    if (!return_url.startsWith("http")) {
      return new Response(JSON.stringify({
        error: "return_url must be a full URL"
      }), {
        status: 400,
        headers
      });
    ...
    const { data: assignment } = await supabase.from("algo_user_assignments").select("openalgo_username, status").eq("user_id", user.id).maybeSingle();
    ...
    if (!OPENALGO_URL || !OPENALGO_APP_KEY) {
      return new Response(JSON.stringify({
        error: "Broker integration not configured. Contact support."
      }), {
        status: 503,
        headers
      });
    }
    const platformUrl = `${OPENALGO_URL}/api/v1/platform/zerodha/login-url` + `?username=${encodeURIComponent(openalgoUsername)}` + `&return_url=${encodeURIComponent(return_url)}`;
    ...
    try {
      ...
      if (res.ok) {
        ...
        return new Response(JSON.stringify({
          error: data?.error ?? "OpenAlgo did not return a URL"
        }), {
          status: 502,
          headers
        });
      ...
      return new Response(JSON.stringify({
        error: `OpenAlgo error ${res.status}: ${errText.slice(0, 200)}`
      }), {
        status: 502,
        headers
      });
      ...
      clearTimeout(timeoutId);
      return new Response(JSON.stringify({
        error: "Could not reach broker backend. Try again shortly."
      }), {
        status: 503,
        headers
      });
