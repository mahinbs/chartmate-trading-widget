...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const uniqueSymbols = allSymbols.filter((symbol, index, arr)=>arr.findIndex((s)=>s.full_symbol === symbol.full_symbol) === index);
...
function getDisplaySymbol(fullSymbol) {
  ...
  return fullSymbol.replace(/\.(NS|BO)$/, '') // Indian exchanges
  .replace(/=X$/, '') // Forex
  .replace(/\^/, '') // Indices
  .replace(/-USD$/, '') // Crypto
  .replace(/=F$/, ''); // Futures
