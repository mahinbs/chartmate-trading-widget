...
function AiAnalysisContent({ broker }: { broker: string }) {
  ...
  const historyIdFromQuery = new URLSearchParams(location.search).get(
    "historyId",
  );
            ...
            <div ref={scannerContainerRef} className="relative">
              <div className="relative">
                ...
                <Input
                  ...
                  onFocus={() =>
                    scannerResults.length > 0 && setScannerDropdownOpen(true)
                  }
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-zinc-900 border border-zinc-700 rounded-lg shadow-2xl z-50 overflow-hidden max-h-64 overflow-y-auto">
                  {scannerResults.map((item, i) => (
                    <button
                      ...
                      onMouseDown={() =>
                        handleScannerSelect(item.full_symbol || item.symbol)
                      }
                    >
          ...
          <Card className="border-zinc-800 bg-zinc-900/30">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-sm text-zinc-400">
                Search a stock above to scan entry & exit points
              </p>
              <p className="text-xs text-zinc-600 mt-1">
                Select strategies, run the scan, and see AI-scored BUY/SELL
                signals with LIVE recommendations
              </p>
            </CardContent>
          </Card>
        )}
  ...
  return (
    <DashboardShellLayout>
      <TradingDashboardAccessGate>
        {(ctx) => <AiAnalysisContent broker={ctx.broker} />}
      </TradingDashboardAccessGate>
    </DashboardShellLayout>
  );
}
