...
function SymbolSearchInput({
  ...
  const search = useCallback(async (q: string) => {
    ...
    try {
      ...
      // Filter to Indian markets only (OpenAlgo works with Indian brokers)
      const indian = data.filter(d =>
        d.full_symbol?.endsWith(".NS") ||
        d.full_symbol?.endsWith(".BO")
      );
  ...
  const handleSelect = (item: SymbolResult) => {
    const exchange = item.full_symbol?.endsWith(".BO") ? "BSE" : "NSE";
    onSelect(item.symbol, exchange);
    setOpen(false);
    setResults([]);
  };
...
// ── PlaceOrderPanel ────────────────────────────────────────────────────────────
function PlaceOrderPanel({ broker, onOrderPlaced }: { broker: string; onOrderPlaced: () => void }) {
          ...
          <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-amber-500/5 border border-amber-500/15 text-[11px]">
            ...
            <p className="text-amber-300/80 leading-relaxed">
              {market.session === "post_market"
                ? "Market is closed. Orders placed now are AMO (After-Market Orders) — they queue and execute at the next market open."
          ...
          <div className="grid grid-cols-2 gap-2">
            {needsLimit && (
              <div className="space-y-1">
                <Label className="text-zinc-500 text-[11px]">Limit Price *</Label>
                    <Input
                  type="number" step="0.05" placeholder="2450.50"
                  value={order.price}
            ...
            {needsTrigger && (
              <div className="space-y-1">
                ...
                <Input
                  type="number" step="0.05" placeholder="2440.00"
    ...
    <div className="min-h-screen bg-black text-zinc-100">
      ...
      <main className="container mx-auto px-4 py-5 max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-5 items-start">
          ...
          {/* ── Right: Sticky order + strategy panels ── */}
          <div className="space-y-4 lg:sticky lg:top-[73px] lg:max-h-[calc(100vh-90px)] lg:overflow-y-auto lg:pb-4" style={{ scrollbarWidth: "thin", scrollbarColor: "#27272a transparent" }}>
            <PlaceOrderPanel
              broker={broker}
              onOrderPlaced={() => setPortfolioKey(k => k + 1)}
            />
            <StrategiesPanel broker={broker} />
          </div>
        </div>
      </main>
    </div>
